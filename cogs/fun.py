...
class TicTacToeButton(discord.ui.Button['TicTacToe']):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if view.current_player == view.X:
            if interaction.user != self.player1: return await interaction.client.ext.warn(interaction, "You cannot interact with this", ephemeral=True)
            ...
            if interaction.user != self.player2: return await interaction.client.ext.warn(interaction, "You cannot interact with this", ephemeral=True)
...
class fun(commands.Cog):
    ...
    @commands.command(description="pack a member", usage="[member]", help="fun")
    async def pack(self, ctx: commands.Context, member: discord.Member):
        ...
        await ctx.send(f"{member.mention} {data['insult']}", allowed_mentions=discord.AllowedMentions(users=False))
    ...
    @commands.command(description="say a message", usage="[message]", help="fun")
    ...
    @commands.command(aliases=["ttt"], description="play tictactoe with a friend", usage="[member]", help="fun")
    async def tictactoe(self, ctx: commands.Context, *, member: discord.Member):
      ...
      yes = discord.ui.Button(emoji=self.bot.yes, style=discord.ButtonStyle.gray)
      no = discord.ui.Button(emoji=self.bot.no, style=discord.ButtonStyle.gray)
      
      async def yes_callback(interaction: discord.Interaction): 
         if interaction.user != member: return await interaction.warn("You are not the author of this embed")
    ...
    @commands.command(description="sends a definition of a word", usage="[word]", help="fun")
    async def urban(self, ctx, *, word):
      ...
      except Exception as e: await ctx.warn("No definition found for **{}**".format(word))
    ...
    async def mtball(self, ctx: commands.Context, *, arg):      
     ...
     return await ctx.neutral(f"Question: {arg}\nAnswer: {random.choice(rand)}")
    
    @commands.command(description="ask a question to ben", usage="[question]", help="fun")
    ...
    @commands.command(description="check how many bitches an user has", usage="<member>", help="fun")
    ...
    async def blacktea(self, ctx: commands.Context):
     try:
      if BlackTea.MatchStart[ctx.guild.id] is True:
       return await ctx.warn("Someone is already playing blacktea in this server")
     ...
     if len(players) < 2:
      ...
      return await ctx.send(content=f"{ctx.author.mention}", embed=discord.Embed(color=self.bot.color, description="😦 not enough players joined to start blacktea"))
     
     while len(players) > 1:
      for player in players:
       strin = await BlackTea.get_string()
       await ctx.send(content=f"<@{player}>", embed=discord.Embed(color=self.bot.color, description=f"⏰ type a word containing **{strin.upper()}** in **10 seconds**"))
        ...
        message = await self.bot.wait_for('message', timeout=10, check=is_correct)
          ...
          try:
            ...
            if BlackTea.lifes[player] == 3:
              await ctx.send(content=f"<@{player}>", embed=discord.Embed(color=self.bot.color, description=f"☠ you are eliminated"))
              BlackTea.lifes[player] = 0
              players.remove(player)
              continue
          except KeyError:
            BlackTea.lifes[player] = 0
          await ctx.send(content=f"<@{player}>", embed=discord.Embed(color=self.bot.color, description=f"💥 you didn't reply on time! **{2-BlackTea.lifes[player]}** lifes remaining"))
          continue
       i=0
       ...
       if i == 0:
          try:
            ...
            BlackTea.lifes[player] = 0
          await ctx.send(content=f"<@{player}>", embed=discord.Embed(color=self.bot.color, description=f"💥 incorrect word! **{2-BlackTea.lifes[player]}** remaining"))
       else: 
         await message.add_reaction("✅")
         i=0
      
     await ctx.send(content=f"👑 <@{players[0]}> won the game")
