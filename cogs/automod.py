...
from tools.helpers import has_perms
...
class automod(commands.Cog):
    ...
    async def invite_edit(self, before, after: discord.Message): 
     ...
     if await has_perms(await self.bot.get_context(message), "manage_guild"): return 
     ...
     if any(invite in message.content for invite in invites):
        ...
        if check is not None: 
          ...
          try:
           ...
           await message.channel.send(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.warn} {message.author.mention}: You have been muted for **1 minute** - sending invite links"))
     ...
     if any(invite in message.content for invite in invites):
        ...
        if check is not None: 
          ...
          try:
           ...
           await message.channel.send(embed=discord.Embed(color=self.bot.color, description=f"{self.bot.warn} {message.author.mention}: You have been muted for **5 minutes** - sending invites links"))
          except KeyError: pass  
    ...
    @anti_invite.command(name="enable", description="enable anti invite", aliases=["e"], brief="manage guild", help="automod") 
    ...
    async def antiinvite_enable(self, ctx: commands.Context):      
        ...
        return await ctx.approve("Enabled anti-invite")
    ...
    async def antiinvite_disable(self, ctx: commands.Context): 
        ...
        return await ctx.approve("Disabled anti-invite")
